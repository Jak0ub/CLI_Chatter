...
def send_msg(server,msg,other_side_key):
    ...
    x = rq.post(f"http://{server}/data", data=msg_enc) #Send message
    if (x.text).split("\n")[0] != "OK":
    ...
    while True:
        ...
        try:
            ...
            if msg.decode("utf-8").lower() == "/quit": #Recieved /quit
                ...
                x = rq.post(f"http://{server}/quit", data=data_encrypted)
                if (x.text).split("\n")[0] == "OK": 
                    print("Other side has left the room. All data was deleted!")
                    others.quit_all()
        except:
...
def mitm_possible(room, server, server_key):
    ...
    #Delete all logs
    data_encrypted = crypto.encrypt(server_key, lines)
    rq.post(f"http://{server}/quit", data=data_encrypted)
    others.quit()

def send_data(server, server_key, lines):
    if type(lines) == list: lines = "\n".join(lines)
    data_encrypted = crypto.encrypt(server_key, lines)
    return rq.post(f"http://{server}/data", data=data_encrypted)
...
def main():
    ...
    if access == True:
        ...
        x = crypto.send_key(f"http://{server}/key", public_key,access_code)
        other_side_response = ""
            ...
            while other_side_response != "start":
                ...
                if request.lower() == "y":
                    ...
                    if other_side_response == "start":
                        ...
                        x = crypto.send_key(f"http://{server}/key", public_key_room,room_password)
                        check_timeout(x)
                        others.clear(clear_cmd)
                        try:
        ...
        t = threading.Thread(target=load_new_msg, args=(server, room, other_side_ip, private_key_room,server_key))
